...
data1<-read.csv("Test_346_CMSY_4category_add.csv")
md.pattern(data1) 
...
data11$Bstart_class<-as.factor(data11$Bstart_class)
...
f1<-vip(bst_model,num_features = 35) 

f<-vi(bst_model)
f<-as.data.frame(f)
length(f$Variable)
ggsave(f1,filename="CMSY_VI_notemp.png",width=10,height=6)
ggsave(f1,filename="CMSY_VI_notemp.pdf",width=10,height=6)
write.csv(f,"VI_0304_notemp.csv")
